...
namespace System.Runtime
{
    static internal class Closure
    {
        ...
        public sealed class Operation
        {
            ...
            public Operation(IntPtr method, Signature signature, Type type)
            {
                var _type = Closure.m_Module.DefineType(string.Concat(Metadata<Type>.Type.Name, Guid.NewGuid().ToString("N")), TypeAttributes.Class | TypeAttributes.Public, Metadata<object>.Type);
                var _constructor = _type.DefineConstructor(MethodAttributes.Public, CallingConventions.HasThis, signature).GetILGenerator();
                var _method = _type.DefineMethod(Metadata<Operation>.Type.Name, MethodAttributes.Public, CallingConventions.HasThis, Metadata.Void, Type.EmptyTypes).GetILGenerator();
                ...
                else
                {
                    ...
                    _constructor.Emit(OpCodes.Ret);
                    _method.Emit(method, type, signature);
                    _method.Emit(OpCodes.Stfld, _type.DefineField(Metadata<Operation>.Field(_Action => _Action.Value).Name, type, FieldAttributes.Public));
                    _method.Emit(OpCodes.Ret);
                    this.Type = _type.CreateType();
                    this.Constructor = this.Type.GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly)[0];
                    this.Method = this.Type.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly)[0];
                    this.Value = this.Type.GetField(Metadata<Operation>.Field(_Action => _Action.Value).Name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly);
                }
            }
        }
    }
